...
class UserTestCase(BaseTestCase):
    ...
    def test_get_account_page(self):
        ...
        with self.client:
            ...
            self.assertIn(u"Раді бачити, test_user", response.data.decode('utf8'))
...
class HomePageTestCase(BaseTestCase):
    ...
    def test_about_view(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Nazar', response.data)
...
class TodoTestCase(BaseTestCase):
    def test_todo_create(self):
        data = {
            'new_task': 'Write flask test123 123 123',
            'description': 'imao imao',
        }
        with self.client:
            response = self.client.post(
    ...
    def test_post_update(self):
        ...
        with self.client:
            ...
            response = self.client.post(
                ...
                data=dict(
                    title="Clash royale", text="11", type="Other", enabled=None, user_id=user.id, category=category_2.id
                ),
                follow_redirects=True
            )
            self.assertEqual(response.status_code, 200)
            ...
            assert post.title == "Clash royale"
